...
- **A real data source now exists** (it didn't when this section was first written): `agent/node_daemon.py` reports per-user traffic (`amnezia_peer_traffic` from `awg show transfer`, `xray_user_traffic` from Xray's StatsService) over `/api/v1/health`; `NodeOrchestrator`'s health-poll loop (also newly wired — `orchestrator.start()` itself was never called before this either, see below) picks it up and, **only if `NodeOrchestrator.set_accounting_engine(engine)` has been called**, forwards it to `TrafficAccountingEngine.ingest_node_traffic()`, which turns the node's cumulative counters into per-user deltas and calls `ingest_traffic()`. Nothing calls `set_accounting_engine()` today — that one call (plus constructing the engine and calling `.start()`) is the entire remaining "~5 lines" to flip this on; it's just as deliberately not done as the rest of this section.
...
- **`TrafficAccountingEngine.__init__` now takes `dry_run: bool = True`** (defaults on). In dry-run, usage counters (`data_used_bytes`/`current_period_usage_bytes`) still update normally every cycle — that's just metering — but no user or agent is ever actually suspended or dropped from nodes; over-quota accounts are logged (`[DRY RUN] ... would suspend ...`) instead. This is the safe way to let the engine run against real data and observe what it *would* do before trusting it to act — construct it, call `.start()`, wire `orchestrator.set_accounting_engine(engine)`, watch the logs for a while, and only then re-construct with `dry_run=False` (or flip `engine.dry_run = False` on the live instance) once satisfied.
